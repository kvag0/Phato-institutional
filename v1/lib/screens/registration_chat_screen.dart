...
class RegistrationChatScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<RegistrationChatScreen> createState() =>
      _RegistrationChatScreenState();
}

class _RegistrationChatScreenState
    extends ConsumerState<RegistrationChatScreen> {
  ...
  void _handleSendMessage({String? preDefinedText}) async {
    ...
    switch (_currentStep) {
      ...
      case RegistrationStep.awaitingAge:
        ...
        if (age == null || age <= 0) {
          _addBotMessage(
              'Ups, isso não parece uma *idade válida*. Por favor, insira apenas números.');
        } else {
          _age = age;
      ...
      case RegistrationStep.awaitingPasswordConfirmation:
        if (userInput != _password) {
          ...
          _addBotMessage(
              'Perfeito! A finalizar o seu registo, por favor aguarde...');

          try {
            final authService = ref.read(authServiceProvider);
            // MUDANÇA AQUI: Passamos os novos dados (name e age) para a função
            await authService.registerWithEmailAndPassword(
                _email, _password, _name, _age!);
            ...
            _addBotMessage(
                'Ocorreu um *erro*: ${e.toString().split('] ').last}. Vamos tentar de novo.');
  ...
  void _addBotMessage(String text,
      {int delay = 1200, Widget? child, bool isBold = false}) {
  ...
  void _askForName({int delay = 1200}) =>
      _addBotMessage('Qual é o teu nome?', delay: delay, isBold: true);
  void _askForAge() =>
      _addBotMessage('Prazer, $_name! E qual é a tua idade?', delay: 1200);
        ...
        SizedBox(
          ...
          child: OutlinedButton.icon(
            ...
            onPressed: () => ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('Login com Apple (mock)!'))),
